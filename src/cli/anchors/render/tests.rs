// Responsibility: cli-anchors-render-tests
use crate::model::{
...
fn proof_run_uses_fallback_when_only_soft_evidence_exists() {
    let report = ProofReport {
        ...
        coverage: None,
        wiring: Vec::new(),
        verification_topology: crate::model::VerificationTopology::default(),
...
fn run_plan_rejects_unsafe_test_like_script_names() {
    ...
    let error = planned_run_commands(&plan, false).expect_err("unsafe scripts should fail closed");
...
fn run_plan_rejects_mutating_and_watch_package_scripts() {
    ...
    let error = planned_run_commands(&plan, false).expect_err("mutating/watch scripts should fail");
...
fn run_plan_resolves_self_command_to_current_executable() {
    let command =
        resolve_run_command("codemap boundaries --changed").expect("self command should resolve");
