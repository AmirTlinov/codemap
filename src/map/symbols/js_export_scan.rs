// Responsibility: map-symbols-js-export-scan
use crate::map::{
    default_export_symbol_name, is_identifier_byte, js_code_line_without_strings_and_comments,
    js_regex_literal_can_start, js_regex_literal_end, local_named_export_bindings,
    matching_symbols,
};
...
pub(crate) fn js_map_keyword_positions(text: &str, keyword: &str) -> Vec<usize> {
...
pub(crate) fn js_map_keyword_at(bytes: &[u8], index: usize, keyword: &[u8]) -> bool {
    bytes
        .get(index..)
...
pub(crate) fn js_map_skip_ascii_whitespace(text: &str, mut index: usize) -> usize {
    let bytes = text.as_bytes();
    while bytes
        .get(index)
...
pub(crate) fn js_map_skip_trivia(text: &str, mut index: usize) -> usize {
    let bytes = text.as_bytes();
    loop {
...
pub(crate) fn local_named_export_bindings_from_statement(
    statement: &str,
...
pub(crate) fn file_has_local_value_shadow(file: &FileInfo, local: &str) -> bool {
    file.local_bindings.contains(local) || file.symbols.iter().any(|symbol| symbol.name == local)
}

pub(crate) fn symbol_is_exported(project: &Project, file_rel: &str, symbol_name: &str) -> bool {
...
pub(crate) fn symbol_exported_under_public_name(
    project: &Project,
    file_rel: &str,
    symbol_name: &str,
    public_name: &str,
