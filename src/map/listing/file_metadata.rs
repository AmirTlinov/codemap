// Responsibility: map-listing-file-metadata
use crate::map::same_symbol_reference_scope;
use crate::model::{FileInfo, Project};
use crate::repo;

pub(crate) fn same_scope_file_references_symbol(
    anchor: &FileInfo,
    file: &FileInfo,
    symbol_name: &str,
) -> bool {
...
pub(crate) fn structural_roles_for_ls(info: &FileInfo) -> Vec<String> {
    info.roles.iter().cloned().collect()
}

pub(crate) fn package_name_for_file(project: &Project, rel: &str) -> Option<String> {
    project
        .packages
        .iter()
        .filter(|package| {
...
pub(crate) fn file_kind_for_ls(info: &FileInfo) -> String {
    for role in [
        "snapshot",
        "golden",
        "asset",
...
pub(crate) fn is_generic_noise(info: &FileInfo) -> bool {
