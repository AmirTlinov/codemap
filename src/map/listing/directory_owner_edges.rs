// Responsibility: map-listing-directory-owner-edges
use crate::map::{
    ci_run_steps, command_invokes_script_surface, command_target, directory_edge_endpoint_at_depth,
    env_declared_keys, files_under_directory, include_package_script_command_edge,
    is_support_artifact_path, lockfiles_for_package, manifest_script_is_proof_relevant,
    owner_workspace_manifest_edges, package_for_rel, package_json_scripts, path_under_scope,
    schema_owner_directory, schema_owner_path, script_target_for_package, script_target_for_path,
    should_hide_owner_edge_path, structural_edge_with_locations, validation_command_like,
    workspace_edge_directory_target, workspace_manifest_file,
};
use crate::model::{EvidenceLocation, EvidenceStrength, Project, StructuralEdge};

pub(crate) fn current_level_owner_edges(
    project: &Project,
    rel: &str,
...
fn current_level_ci_edges(
    ...
    for ci in project
        .files
        .values()
        .filter(|file| file.has_role("build_ci"))
    {
