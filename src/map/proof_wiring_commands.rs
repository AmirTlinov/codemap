fn resolve_proof_command_fact(
    ...
    if matches!(parsed.runner.as_str(), "cargo" | "swift" | "go" | "pytest" | "python" | "python3" | "node" | "npx" | "tsx" | "vitest" | "jest" | "playwright") {
        return proof_wiring_fact(
            ("runner", "wired"),
            command.to_string(),
            proof.path.clone(),
    ...
    proof_wiring_fact(
        ...
        command.to_string(),
        proof.path.clone(),
        format!("runner `{}` is not in the static runner table", parsed.runner),
...
fn resolve_make_like_command(
    ...
    let matched = project.scripts.iter().find(|script| {
    ...
    if let Some(script) = matched {
        return proof_wiring_fact(
            ...
            script
                .path
                .as_ref()
                .map(|path| EvidenceLocation::line(path, script.line_start.unwrap_or(1), "script_target"))
                .into_iter()
                .collect(),
            Some(format!(
