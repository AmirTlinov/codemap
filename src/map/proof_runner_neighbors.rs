fn proof_runner_neighbor_edges(project: &Project, rel: &str) -> Vec<StructuralEdge> {
    ...
    unique_structural_edges(edges).into_iter().take(12).collect()
}
...
fn proof_neighbor_file_location(
    ...
    if let Some(line) = first_line_containing_ci(project, rel, &token_refs) {
        vec![EvidenceLocation::line(rel, line, "proof_neighbor_token_match")]
    } else {
