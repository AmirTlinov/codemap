// Responsibility: map-proof-precedence
use crate::model::{EvidenceStrength, ProofSurface};
use crate::proof_classification::proof_base_evidence;
use std::collections::BTreeMap;
use std::collections::VecDeque;

pub(crate) fn unique_proof_surfaces(values: Vec<ProofSurface>) -> Vec<ProofSurface> {
...
pub(crate) fn unique_proof_commands(values: Vec<ProofSurface>) -> Vec<ProofSurface> {
    let mut seen = BTreeMap::new();
    let mut out = Vec::new();
    for value in values {
...
pub(crate) fn balanced_proof_surface_prefix(
    values: &[ProofSurface],
    limit: usize,
) -> Vec<ProofSurface> {
    if values.len() <= limit {
...
pub(crate) fn proof_surface_precedence(value: &ProofSurface) -> (EvidenceStrength, usize) {
    (value.strength, proof_evidence_precedence(&value.evidence))
}

pub(crate) fn proof_surface_satisfies_specific_proof(proof: &ProofSurface) -> bool {
    crate::proof_classification::proof_surface_is_runnable_validation(proof)
}

pub(crate) fn proof_surface_is_soft_structural_match(proof: &ProofSurface) -> bool {
    matches!(
...
pub(crate) fn proof_surface_command_closes_fallback(proof: &ProofSurface) -> bool {
...
pub(crate) fn proof_evidence_precedence(evidence: &str) -> usize {
