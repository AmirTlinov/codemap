// Responsibility: map-proof-owner-ci-script-body
use crate::map::{
    CiOwnerStepKind, CiRunStep, ci_owner_command_has_unsupported_shell_composition,
    ci_owner_command_has_unsupported_shell_control, ci_owner_command_is_non_validation,
    ci_owner_script_name_is_validation, ci_owner_step_kind, ci_owner_validation_step_reason,
    command_references_package, command_tokens, manifest_script_command_body_is_run_safe,
    package_json_scripts, strip_inline_shell_comment, token_invokes_package_script, unique,
};
...
pub(crate) fn ci_owner_proof_surface_for_step(
    project: &Project,
    ci_rel: &str,
    step: CiRunStep,
) -> Option<ProofSurface> {
...
pub(crate) fn ci_owner_step_kind_for_project(
    project: &Project,
    command: &str,
) -> Option<CiOwnerStepKind> {
...
fn ci_owner_matching_package_scripts<'a>(
        ...
        .filter(|package| package.ecosystem == "javascript")
    {
        ...
        for (name, body, line) in package_json_scripts(project, &package.manifest) {
            if invoked
                .iter()
                .any(|script| script == &name.to_ascii_lowercase())
            {
