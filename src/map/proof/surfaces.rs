// Responsibility: map-proof-surfaces
use crate::map::{
    contract_link_edges, cross_boundary_consumer_edges, direct_consumer_edges,
    directory_seed_file_paths, files_under_directory, impact_level_for_file, is_generic_noise,
    mediated_proof_strength, owner_surface_proof_surfaces, proof_command_for_test,
    proof_edges_via_direct_dependencies, proof_surface_locations_for_target,
    proof_surface_satisfies_specific_proof, role_aware_command_proof_surfaces, sort_edges,
    strict_test_edges_for_file, structural_impact_level, symbol_anchor_path,
    symbol_proof_edges_with_owning_file, symbol_reference_edges, unique,
};
use crate::model::{Project, ProofSurface, Risk};

pub(crate) fn proof_surfaces_for_anchor(
    project: &Project,
    anchor: &str,
    depth: usize,
    limit: usize,
            ...
            .take(limit)
        {
                ...
                .take(limit)
            {
                    ...
                    strict_test_edges_for_file(project, &second.from, limit)
                {
                    ...
                    let locations =
                        proof_surface_locations_for_target(project, from, &test, &evidence);
                    out.push(ProofSurface {
                        command: proof_command_for_test(project, &test),
                        locations,
                        path: Some(test),
...
pub(crate) fn proof_surfaces_for_directory(
    project: &Project,
    rel: &str,
    depth: usize,
    limit: usize,
...
pub(crate) fn proof_surfaces_for_file_paths(
    project: &Project,
    files: &[String],
    depth: usize,
    limit: usize,
...
pub(crate) fn proof_surfaces_for_symbol_anchor(
    project: &Project,
    file_rel: &str,
    symbol_name: &str,
    depth: usize,
        ...
        .take(limit)
    {
        ...
        for (test, evidence, strength) in strict_test_edges_for_file(project, &consumer_file, limit)
        {
            out.push(ProofSurface {
                command: proof_command_for_test(project, &test),
                locations: proof_surface_locations_for_target(
                    project,
                    &consumer_file,
                    &test,
                    &evidence,
...
pub(crate) fn impact_level_for_directory(project: &Project, rel: &str, depth: usize) -> Risk {
...
pub(crate) fn structural_impact_level_for_file(
    project: &Project,
    rel: &str,
    depth: usize,
) -> (Risk, Vec<String>) {
...
fn proof_reason_for_evidence(evidence: &str, scope: &str) -> String {
    ...
    match evidence {
        ...
        "test_role_surface_match" => {
            format!("test shares a deterministic source role with {scope}")
        }
