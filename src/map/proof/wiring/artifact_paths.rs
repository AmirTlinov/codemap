// Responsibility: map-proof-wiring-artifact-paths
use crate::map::{
    ParsedProofCommand, package_for_parsed_command, package_json_scripts,
    package_script_name_from_command, parse_static_command, unquote_shell_token,
};
use crate::model::{EvidenceStrength, Project, ProofSurface};
use crate::repo;
use std::collections::BTreeSet;

pub(crate) fn artifact_paths_for_command_or_file(
    project: &Project,
    command: &str,
    path: Option<&str>,
) -> Vec<String> {
...
fn collect_artifact_path_tokens(text: &str, out: &mut BTreeSet<String>) {
        ...
        .split(|ch: char| ch.is_whitespace() || matches!(ch, '"' | '\'' | '`' | ',' | ')' | '('))
    {
        if token_is_artifact_path(token) {
            out.insert(repo::normalize_rel_path(
                token.trim_matches(['\'', '"', '`']),
            ));
        }
    }
