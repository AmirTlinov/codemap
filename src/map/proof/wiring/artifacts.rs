// Responsibility: map-proof-wiring-artifacts
use crate::map::{
    artifact_consumers, artifact_paths_for_command_or_file, declared_receipt_fields,
    field_consumers_from_texts, field_line_locations, file_has_predicate_language,
    json_string_field, markdown_declared_fields, proof_surface_wiring_facts,
    proof_wiring_consumer_texts, proof_wiring_expand_for_proof, proof_wiring_fact,
    receipt_field_is_execution, receipt_field_is_schema, shell_quote, text_has_predicate_language,
};
use crate::model::{EvidenceLocation, EvidenceStrength, Project, ProofSurface, ProofWiringFact};
use std::collections::BTreeSet;

pub(crate) fn artifact_chain_facts_for_command(
    project: &Project,
    command: &str,
    proof: &ProofSurface,
    selector: &str,
...
pub(crate) fn artifact_contract_wiring_facts_limited(
    project: &Project,
    anchor: &str,
    selector: &str,
    limit: usize,
    ...
    facts.extend(receipt_declared_command_wiring_facts(
        project, anchor, selector,
    ));
    facts.truncate(limit);
    if facts.len() >= limit {
        return facts;
...
fn receipt_declared_command_wiring_facts(
    ...
    let proof = ProofSurface {
        ...
        locations: field_line_locations(
            &text,
            rel,
            &["proof_command", "command", "validation_command"],
        ),
    };
