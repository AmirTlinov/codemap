// Responsibility: proof-map-lens-helpers
use crate::map::{expand_with_concrete_limit, proof_surface_precedence};
use crate::model::{HiddenGroup, ProofSurface, Surface, Unknown};
use std::collections::BTreeMap;
use std::collections::BTreeSet;

pub(crate) fn group_duplicate_proof_surfaces(
...
pub(crate) fn group_duplicate_missing_surfaces(
...
pub(crate) fn group_duplicate_unknowns(
...
pub(crate) fn proof_map_expand(selector: &str, raw_sensors: bool) -> String {
    let raw = if raw_sensors { " --raw-sensors" } else { "" };
    format!("codemap proof-map {selector}{raw} --limit <larger-number>")
}

pub(crate) fn proof_map_proof_expand(selector: &str) -> String {
