// Responsibility: changed-lens-truth-paths
use crate::map::changed_map_path_file_name;
use crate::model::Project;
use crate::repo;
use std::path::Path;

pub(crate) fn changed_path_is_generated(project: &Project, path: &str) -> bool {
...
pub(crate) fn changed_path_is_large_binary(project: &Project, path: &str) -> bool {
    const LARGE_BINARY_BYTES: u64 = 10 * 1024 * 1024;
    let size = project.files.get(path).map(|file| file.size).or_else(|| {
        std::fs::metadata(project.root.join(path))
            .ok()
            .map(|meta| meta.len())
    });
...
pub(crate) fn changed_path_is_model_weight_like(path: &str) -> bool {
    let lower = path.to_ascii_lowercase();
...
pub(crate) fn changed_path_is_protected_looking(path: &str) -> bool {
    let lower = path.to_ascii_lowercase();
...
pub(crate) fn changed_path_is_instruction_file(path: &str) -> bool {
    let lower = path.to_ascii_lowercase();
...
pub(crate) fn changed_manifest_for_lockfile(path: &str) -> Option<String> {
    let lower = path.to_ascii_lowercase();
...
pub(crate) fn changed_lockfiles_for_manifest(project: &Project, path: &str) -> Vec<String> {
    let lower = path.to_ascii_lowercase();
    let dir = changed_path_dir(path);
...
pub(crate) fn changed_path_is_runner_like(path: &str) -> bool {
    let lower = path.to_ascii_lowercase();
    let name = changed_map_path_file_name(&lower);
...
pub(crate) fn changed_lens_path_looks_like_source(path: &str) -> bool {
    matches!(
...
pub(crate) fn changed_runner_has_package_script(project: &Project, path: &str) -> bool {
