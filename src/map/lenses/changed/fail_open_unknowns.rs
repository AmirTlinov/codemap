// Responsibility: changed-lens-fail-open-unknowns
use crate::map::{
    env_declared_keys, owner_ci_edges, owner_env_unknowns, owner_schema_edges,
    owner_schema_incoming_edges, owner_surface_proof_surfaces, schema_owner_path, shell_quote,
    status_file_name, strict_test_edges_for_file, unknown, unknown_ci_validation_step_not_found,
    workspace_manifest_file, workspace_manifest_member_packages,
};
use crate::model::{EvidenceStrength, FileInfo, Project, ProofSurface, Unknown};
use crate::repo;

pub(crate) fn changed_fail_open_unknowns(project: &Project, changed: &[String]) -> Vec<Unknown> {
...
pub(crate) fn file_uses_ci_run_step_syntax(file: &FileInfo) -> bool {
...
pub(crate) fn proof_ci_run_step_is_validation(proof: &ProofSurface) -> bool {
...
pub(crate) fn changed_should_check_direct_proof(file: &FileInfo) -> bool {
...
pub(crate) fn changed_manifest_unknowns(project: &Project, rel: &str) -> Vec<Unknown> {
        ...
        .any(|edge| edge.to_manifest.as_deref() == Some(rel))
    {
        unknowns.push(unknown(
            ...
            Some(format!(
                "codemap graph --path {} --lens causal",
                shell_quote(rel)
            )),
        ));
    }
...
pub(crate) fn changed_schema_unknowns(project: &Project, rel: &str) -> Vec<Unknown> {
    ...
    if !edges
        .iter()
        .any(|edge| edge.edge_type == "schema_migration")
    {
        unknowns.push(unknown(
...
pub(crate) fn changed_env_unknowns(project: &Project, rel: &str) -> Vec<Unknown> {
    let proofs = owner_surface_proof_surfaces(project, rel);
    let mut unknowns = Vec::new();
