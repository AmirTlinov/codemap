...
fn cargo_manifest_builtin_proof_surfaces(project: &Project, file: &FileInfo) -> Vec<ProofSurface> {
    ...
    let line = first_line_containing(project, &file.rel, &["[package]", "[workspace]"]).unwrap_or(1);
...
fn workspace_manifest_script_proof_surfaces(
    project: &Project,
    file: &FileInfo,
) -> Vec<ProofSurface> {
    ...
    package_json_scripts(project, &root_manifest)
        ...
        .map(|(name, command, line)| ProofSurface {
            ...
            locations: vec![EvidenceLocation::line(&root_manifest, line, "workspace_script")],
        })
        .collect()
}

fn workspace_manifest_ci_reference_proof_surfaces(
    project: &Project,
    file: &FileInfo,
) -> Vec<ProofSurface> {
    ci_run_reference_proof_surfaces(project, file, "workspace_manifest_ci_reference", |command| {
        workspace_ci_run_match_reason(project, &file.rel, command)
    })
}

fn owner_workspace_manifest_edges(project: &Project, rel: &str) -> Vec<StructuralEdge> {
    ...
    for pattern in pnpm_workspace_patterns(project, rel) {
        ...
        edges.push(structural_edge_with_locations(
            ...
            vec![EvidenceLocation::line(rel, pattern.line, "workspace_pattern")],
        ));
    }
    ...
    if let Some(root_manifest) = workspace_root_package_manifest(project, rel) {
            ...
            .filter(|(name, command, _)| workspace_script_is_proof_relevant(name, command))
        {
            edges.push(structural_edge_with_locations(
                ...
                EvidenceStrength::Hard,
                vec![EvidenceLocation::line(&root_manifest, line, "package_script")],
            ));
            edges.push(structural_edge_with_locations(
                format!("script:{name}"),
...
fn workspace_manifest_file(rel: &str) -> bool {
    matches!(
...
fn workspace_manifest_member_packages<'a>(
    project: &'a Project,
    rel: &str,
...
fn workspace_script_is_proof_relevant(name: &str, command: &str) -> bool {
    manifest_script_is_proof_relevant(name, command) || name.to_ascii_lowercase().starts_with("verify:")
}
...
fn command_mentions_workspace_tooling(command: &str) -> bool {
    ...
    tokens.iter().any(|token| matches!(token.as_str(), "turbo" | "pnpm"))
...
fn workspace_pattern_matches_package(workspace_root: &str, pattern: &str, package_path: &str) -> bool {
    let pattern_path = workspace_pattern_repo_path(workspace_root, pattern);
    if let Some(base) = pattern_path.strip_suffix("/*") {
        let Some(rest) = package_path.strip_prefix(&format!("{}/", base.trim_end_matches('/'))) else {
            return false;
        };
...
fn manifest_dir_for_rel(rel: &str) -> String {
    Path::new(rel)
        .parent()
