fn inventory_package_kind(root: &Path, rel: &str) -> Option<String> {
...
fn inventory_recursive_structural_kind(kind: &str, rel: &str) -> bool {
    matches!(
        kind,
        "manifest"
            | "lockfile"
...
fn inventory_root_script_edges(
    root: &Path,
    files: &[String],
    ...
    {
        inventory_command_edges("Cargo.toml", "test", "cargo test", 1, &mut labels, &mut edges);
    }
...
fn inventory_package_json_script_edges(
    ...
    for (name, value) in scripts {
        let Some(command) = value.as_str().map(str::trim).filter(|command| !command.is_empty())
        else {
            continue;
        };
...
fn inventory_workspace_edges(root: &Path, files: &[String]) -> Vec<StructuralEdge> {
    let mut edges = Vec::new();
...
fn inventory_lockfile_name(name: &str) -> bool {
    matches!(
        name,
        "package-lock.json"
            | "npm-shrinkwrap.json"
...
fn inventory_ci_path(rel: &str) -> bool {
...
fn inventory_runtime_config_path(rel: &str, name: &str) -> bool {
    matches!(
        name,
        "dockerfile"
            | "docker-compose.yml"
...
fn inventory_schema_path(rel: &str, ext: &str) -> bool {
    matches!(ext, "prisma" | "graphql" | "gql" | "proto" | "avsc")
        || rel.starts_with("contracts/")
}

fn inventory_migration_path(rel: &str, ext: &str) -> bool {
    ext == "sql" && (rel.starts_with("migrations/") || rel.contains("/migrations/"))
}

fn inventory_support_unit(rel: &str) -> String {
