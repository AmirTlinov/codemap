// Responsibility: repo-js-symbols-jsx-refs
use crate::model::SymbolInfo;
use crate::repo::{
    code_without_comments_or_strings, collect_js_balanced_param_bindings,
    collect_js_param_bindings, extract_go_symbols, extract_js_symbols, extract_python_symbols,
    extract_rust_symbols, extract_swift_symbols, identifier_is_selector_tail, identifier_re,
    is_source_ext, js_destructuring_binding_patterns, js_for_binding_re, js_single_arrow_param_re,
    jsx_tag_re, language_keyword, symbols_with_ranges,
};
use std::collections::BTreeSet;

pub(crate) fn extract_symbols(text: &str, ext: &str) -> Vec<SymbolInfo> {
    if ext == "swift" {
...
pub(crate) fn extract_js_symbols_from_cleaned(
    text: &str,
    cleaned: &str,
    ext: &str,
) -> Vec<SymbolInfo> {
...
pub(crate) fn extract_identifier_references(text: &str, ext: &str) -> BTreeSet<String> {
...
pub(crate) fn extract_identifier_references_from_cleaned(cleaned: &str) -> BTreeSet<String> {
    identifier_re()
        .find_iter(cleaned)
...
pub(crate) fn extract_jsx_tags(text: &str, ext: &str) -> BTreeSet<String> {
...
pub(crate) fn extract_jsx_tags_from_cleaned(cleaned: &str, ext: &str) -> BTreeSet<String> {
    if !matches!(ext, "tsx" | "jsx" | "vue" | "svelte") {
        return BTreeSet::new();
    }
...
pub(crate) fn extract_local_bindings_from_cleaned(cleaned: &str, ext: &str) -> BTreeSet<String> {
    if !matches!(
        ext,
        "ts" | "tsx" | "js" | "jsx" | "mjs" | "cjs" | "vue" | "svelte"
    ) {
