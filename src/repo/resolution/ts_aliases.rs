// Responsibility: repo-resolution-ts-aliases
use crate::model::FileInfo;
use crate::repo::{manifest_dir, normalize_rel_path, resolve_path_like};
...
#[derive(Debug, Clone)]
pub(crate) struct TsPathAlias {
    pub(crate) config_dir: String,
    pub(crate) pattern: String,
    pub(crate) targets: Vec<String>,
}

pub(crate) fn detect_ts_path_aliases(
    root: &Path,
...
pub(crate) fn ts_alias_applies_to_importer(alias: &TsPathAlias, from: &str) -> bool {
    alias.config_dir == "."
        || from == alias.config_dir
        || from.starts_with(&format!("{}/", alias.config_dir.trim_end_matches('/')))
}

pub(crate) fn resolve_ts_path_alias(
    alias: &TsPathAlias,
    spec: &str,
...
pub(crate) fn match_pattern_wildcard(pattern: &str, value: &str) -> Option<Option<String>> {
    if !pattern.contains('*') {
