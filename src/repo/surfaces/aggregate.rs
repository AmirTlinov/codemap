// Responsibility: repo-surfaces-aggregate
use crate::repo::{
    SurfaceExtraction, extract_js_static_imports, js_balanced_call_end,
    js_byte_is_inside_string_or_regex_literal, js_destructure_part_is_direct_shorthand_prop,
    js_last_balanced_object_span, js_split_top_level_commas, line_declares_pending_function_body,
    line_declares_pending_method_body, line_has_playwright_test_call,
    line_has_playwright_test_method_call, line_starts_function_callback_body,
    parse_js_import_clause_bindings, skip_js_whitespace,
};
use std::collections::BTreeSet;

pub(crate) fn merge_surface_extraction(target: &mut SurfaceExtraction, source: SurfaceExtraction) {
    target.tokens.extend(source.tokens);
    target.phrases.extend(source.phrases);
    target.visited_routes.extend(source.visited_routes);
}

pub(crate) fn playwright_test_bindings(text: &str) -> BTreeSet<String> {
...
pub(crate) fn line_declares_playwright_page_fixture(
    line: &str,
...
pub(crate) fn line_declares_disabled_playwright_describe(
    line: &str,
    playwright_test_bindings: &BTreeSet<String>,
) -> bool {
...
pub(crate) fn line_starts_playwright_describe_callback_body(line: &str) -> bool {
    line_starts_arrow_callback_body(line) || line_starts_function_callback_body(line)
}

pub(crate) fn line_starts_nested_playwright_body(line: &str) -> bool {
    line_starts_function_callback_body(line) || line_starts_method_callback_body(line)
}

pub(crate) fn line_has_arrow_callback(line: &str) -> bool {
...
pub(crate) fn line_starts_arrow_callback_body(line: &str) -> bool {
    let mut search_start = 0usize;
    while let Some(relative) = line[search_start..].find("=>") {
        let arrow = search_start + relative;
        if js_byte_is_inside_string_or_regex_literal(line, arrow) {
...
pub(crate) fn line_ends_js_statement(line: &str) -> bool {
...
pub(crate) fn line_declares_pending_nested_body(line: &str) -> bool {
    line_declares_pending_function_body(line) || line_declares_pending_method_body(line)
}

pub(crate) fn line_opens_pending_nested_body(line: &str) -> bool {
    line.trim_start().starts_with('{')
}

pub(crate) fn line_opens_pending_control_flow_body(line: &str) -> bool {
    line.trim_start().starts_with('{')
}

pub(crate) fn line_opens_control_flow_body(line: &str) -> bool {
    line.char_indices()
        .any(|(byte, ch)| ch == '{' && !js_byte_is_inside_string_or_regex_literal(line, byte))
}

pub(crate) fn line_starts_unparsed_playwright_control_flow(line: &str) -> bool {
...
pub(crate) fn line_has_playwright_scope_terminator_before_role_name_call(
    line: &str,
    playwright_test_bindings: &BTreeSet<String>,
) -> bool {
...
pub(crate) fn line_terminates_playwright_page_scope(
    line: &str,
    playwright_test_bindings: &BTreeSet<String>,
) -> bool {
