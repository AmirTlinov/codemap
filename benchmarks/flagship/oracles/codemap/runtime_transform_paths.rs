...
fn response_projection_uses_primary_parameter_and_balanced_return_values() {
    ...
    git(repo.path(), &["config", "user.name", "a"]);
    write(
        &repo.path().join("app/api/account/route.ts"),
        r#"export function GET() {
  csrfObserver();
  return Response.json(sanitizeAccount(loadAccount()));
}

function csrfObserver() {
  observe("csrf");
}

function sanitizeAccount(account: Account, options = {}) {
  const local = options.local;
  if (account.internalToken || account.auditTrail) record(local.auditTrail);
  return {
    id: account.id,
    profile: {
      name: formatName(account.name, { fallback: account.alias }),
      tags: [account.tags, normalize(account.metadata, { compact: true })],
    },
    auditTrail: local.auditTrail,
  };
}
"#,
    );
    ...
    assert!(
        paths.iter().any(|edge| {
            edge["from"]
                .as_str()
                .is_some_and(|from| from.contains("sanitizeAccount"))
                && edge["to"].as_str().is_some_and(|to| {
                    to.ends_with("without(auditTrail,internalToken)")
                })
